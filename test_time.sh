...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=1 python3 train/test_time.py --env_name ${env} --abr_name ${abr} --algorithm_name ${algo} --experiment_name ${exp} \
    --num_agents ${num_agents} --seed ${seed} --n_training_threads 1 --n_rollout_threads 1 \
    --num_mini_batch 1 --episode_length 100 --num_env_steps 20000 --ppo_epoch 15 \
